...
const persons = new Array('James','Maria');     // Array도 객체이므로 생성자 함수를 사용 가능 
console.log(cars, persons);  // []로 만드나,  new를 이용해서 만드나 동일
...
console.log(cars, persons);     // cars.toString() + person.toString()
...
function reverseString(str) {           // 문자열을 뒤집는 함수를 reverse() 사용
    // let strArray = str.split('');        // 이렇게 쓰면 하수?  
    // strArray.reverse();
...
console.log(fruits.slice(1, 2));    // 1인덱스 앞에서 2인덱스 앞까지 자름 / 시작인덱스,끝인덱스(끝은 포함하지 않음)
console.log(fruits.slice(2));       // 끝 인덱스를 생략하면 끝까지 

console.clear();

// 배열의 요소를 정렬(sort) - 배열이 변함
console.log(cars);              // [ 'BMW', 'Volvo', 'Saab' ]
cars.sort();                    // 오름차순 정렬
